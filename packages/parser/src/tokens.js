/* Tokenizers for CSS tokens that can't be
...
function startOfComment(input) {
	let next;
	return (
		input.next == slash && ((next = input.peek(1)) == slash || next == asterisk)
	);
}

export const spaces = new ExternalTokenizer(
	(input, stack) => {
...
export const comments = new ExternalTokenizer((input, stack) => {
	...
	if (input.next == slash) {
		// look ahead for one more slash, in which case it's a Sassdoc comment, which can span several lines
		input.advance();
