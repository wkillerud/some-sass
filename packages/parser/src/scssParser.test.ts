...
import { assertNode, assertError } from "./cssParser.test.js";
...
suite("SCSS - Parser", () => {
	...
	test("Module variable", function () {
		...
		assertNode(
			"module.$co42lor",
			parser,
			parser._parseModuleMember.bind(parser),
		);
		assertNode(
			"module.$-co42lor",
			parser,
	...
	test("VariableDeclaration", function () {
		...
		assertNode(
			"$footer-height: 40px !default !global",
			parser,
			parser._parseVariableDeclaration.bind(parser),
		);
		assertNode(
	...
	test("Expr", function () {
		...
		assertNode(
			"(20 + 20 + 20 + module.$const)",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"(20 + 20 + 20 + 20 + module.$const)",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"(20 + 20 + module.$const + 20 + 20 + module.$const)",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"module.$color, 42%, $color",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"$color, 42%, module.$color",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"module.$color, 42%, module.$color",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"module.$color - ($color + 10%)",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"$color - (module.$color + 10%)",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"module.$color - (module.$color + 10%)",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"not (module.$v and $b) or $c",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"not ($v and module.$b) or $c",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"not ($v and $b) or module.$c",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"not (module.$v and module.$b) or $c",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"not (module.$v and $b) or module.$c",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"not ($v and module.$b) or module.$c",
			parser,
			parser._parseExpr.bind(parser),
		);
		assertNode(
			"not (module.$v and module.$b) or module.$c",
			parser,
			parser._parseExpr.bind(parser),
		);
	...
	test("Declaration", function () {
		...
		assertNode(
			"foo: if($value == 'default', flex-gutter(), $value)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"foo: if(true, !important, null)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"color: selector-replace(&, 1)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		...
		assertNode(
			"dummy: (20 / module.$const)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"dummy: (20 / 20 + module.$const)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"dummy: module.func($red)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"dummy: module.func($red) !important",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"dummy: module.desaturate($red, 10%)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"dummy: desaturate(module.$red, 10%)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"dummy: module.desaturate(module.$red, 10%)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"dummy: module.desaturate(16, 10%)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"color: module.$base-color + #111",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"color: 100% / 2 + module.$ref",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"border: (module.$width * 2) solid black",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"prop-erty: module.fnc($t, 10%)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"prop-erty: fnc(module.$t, 10%)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
			"prop-erty: module.fnc(module.$t, 10%)",
			parser,
			parser._parseDeclaration.bind(parser),
		);
		assertNode(
	...
	test("Stylesheet", function () {
		...
		assertNode(
			"#main { width: 97%; p, div { a { font-weight: bold; } } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"fo { font: 2px/3px { family: fantasy; } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"selector { propsuffix: { nested: 1px; } rule: 1px; nested.selector { foo: 1; } nested:selector { foo: 2 }}",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"@mixin keyframe { @keyframes name { @content; } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@include keyframe { 10% { top: 3px; } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			".class{&--sub-class-with-ampersand{color: red;}}",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertError(
			"fo { font: 2px/3px { family } }",
			parser,
			parser._parseStylesheet.bind(parser),
			ParseError.ColonExpected,
		);
		...
		assertNode(
			"legend {foo{a:s}margin-top:0;margin-bottom:#123;margin-top:m.s(1)}",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@include module.keyframe { 10% { top: 3px; } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
	});
	...
	test("@import", function () {
		...
		assertNode(
			'@import "test.css", "bar.css" screen, projection',
			parser,
			parser._parseImport.bind(parser),
		);
		assertNode(
		...
		assertError(
			...
			parser,
			parser._parseImport.bind(parser),
			ParseError.URIOrStringExpected,
		);
		assertError(
			"@import",
			parser,
	...
	test("@container", function () {
		const parser = new SCSSParser();
		assertNode(
			`@container (min-width: #{$minWidth}) { .scss-interpolation { line-height: 10cqh; } }`,
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			`.item-icon { @container (max-height: 100px) { .item-icon { display: none;  } } }`,
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
	...
	test("@use", function () {
		...
		assertNode(
			'@use "test" with ($foo: "test", $bar: 1)',
			parser,
			parser._parseUse.bind(parser),
		);
		assertNode(
			'@use "test" as foo with ($foo: "test", $bar: 1)',
			parser,
			parser._parseUse.bind(parser),
		);
	...
	test("@forward", function () {
		...
		assertNode(
			'@forward "test" hide this',
			parser,
			parser._parseForward.bind(parser),
		);
		assertNode(
			'@forward "test" hide $that',
			parser,
			parser._parseForward.bind(parser),
		);
		assertNode(
			'@forward "test" hide this $that',
			parser,
			parser._parseForward.bind(parser),
		);
		assertNode(
		...
		assertNode(
			'@forward "abstracts/functions" show px-to-rem, theme-color',
			parser,
			parser._parseForward.bind(parser),
		);
		assertNode(
			'@forward "test" show this',
			parser,
			parser._parseForward.bind(parser),
		);
		assertNode(
			'@forward "test" show $that',
			parser,
			parser._parseForward.bind(parser),
		);
		assertNode(
			'@forward "test" show this $that',
			parser,
			parser._parseForward.bind(parser),
		);
		assertNode(
			'@forward "test" as foo-* show this $that',
			parser,
			parser._parseForward.bind(parser),
		);
		...
		assertNode(
			'@use "lib"; @forward "test"',
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			'@forward "test"; @forward "lib"',
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			'$test: "test"; @forward "test"',
			parser,
			parser._parseStylesheet.bind(parser),
		);
	});
	...
	test("@media", function () {
		...
		assertNode(
			"foo { bar { @media screen and (orientation: landscape) {}} }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertNode(
			".something { @media (max-width: 760px) { > .test { color: blue; } } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			".something { @media (max-width: 760px) { ~ div { display: block; } } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"@media (max-width: 760px) { + div { display: block; } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"@media #{layout.$media} and ($feature: $value)  {}",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@media #{$media} and (layout.$feature: $value)  {}",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@media #{$media} and ($feature: layout.$value)  {}",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"@media screen and (list.nth($query, 1): nth($query, 2)) { }",
			parser,
			parser._parseMedia.bind(parser),
		);
		assertNode(
			"@media screen and (nth(list.$query, 1): nth($query, 2)) { }",
			parser,
			parser._parseMedia.bind(parser),
		);
		assertNode(
			"@media screen and (nth($query, 1): list.nth($query, 2)) { }",
			parser,
			parser._parseMedia.bind(parser),
		);
		assertNode(
			"@media screen and (nth($query, 1): nth(list.$query, 2)) { }",
			parser,
			parser._parseMedia.bind(parser),
		);
		assertNode(
	...
	test("@keyframe", function () {
		...
		assertNode(
			"@keyframes name { @for $i from 0 through $steps { #{$i * (100%/$steps)} { transform: $rotate $translate; } } }",
			parser,
			parser._parseKeyframe.bind(parser),
		); // issue 42086
		assertNode(
			'@keyframes test-keyframe { @for $i from 1 through 60 { $s: ($i * 100) / 60 + "%"; } }',
			parser,
			parser._parseKeyframe.bind(parser),
		);

		assertNode(
			"@keyframes name { @for $i from 0 through m.$steps { #{$i * (100%/$steps)} { transform: $rotate $translate; } } }",
			parser,
			parser._parseKeyframe.bind(parser),
		);
		assertNode(
			"@keyframes name { @function bar() { } }",
			parser,
			parser._parseKeyframe.bind(parser),
		); // #197742
		assertNode(
			"@keyframes name { @include keyframe-mixin(); }",
			parser,
			parser._parseKeyframe.bind(parser),
		); // #197742
	});
	...
	test("@extend", function () {
		const parser = new SCSSParser();
		assertNode(
			".themable { @extend %theme; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"foo { @extend .error; border-width: 3px; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertNode(
			".seriousError {  @extend .error; @extend .attention; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"#context a%extreme { color: blue; }  .notice { @extend %extreme }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@media print { .error {  } .seriousError { @extend .error; } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@mixin error($a: false) { @extend .#{$a}; @extend ##{$a}; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			".foo { @extend .text-center, .uppercase; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertNode(
			".foo { @extend .text-center, .uppercase !optional ; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertError(
	...
	test("@debug", function () {
		...
		assertNode(
			'@function setStyle($map, $object, $style) { @warn "The key ´#{$object} is not available in the map."; @return null; }',
			parser,
			parser._parseStylesheet.bind(parser),
		);
	});
	...
	test("@if", function () {
		...
		assertNode(
			"@if 1 <= $const { border: 3px; } @else { border: 4px; }",
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
		);
		assertNode(
			"@if 1 >= (1 + $foo) { border: 3px; } @else if 1 + 1 == 2 { border: 4px; }",
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
		);
		assertNode(
			"p { @if $i == 1 { x: 3px; } @else if $i == 1 { x: 4px; } @else { x: 4px; } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			'@if (index($_RESOURCES, "clean") != null) { @error "sdssd"; }',
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"@if 1 <= m.$const { border: 3px; } @else { border: 4px; }",
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
		);
		assertNode(
			"@if 1 >= (1 + m.$foo) { border: 3px; } @else if 1 + 1 == 2 { border: 4px; }",
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
		);
		assertNode(
			"p { @if m.$i == 1 { x: 3px; } @else if $i == 1 { x: 4px; } @else { x: 4px; } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"p { @if $i == 1 { x: 3px; } @else if m.$i == 1 { x: 4px; } @else { x: 4px; } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"p { @if m.$i == 1 { x: 3px; } @else if m.$i == 1 { x: 4px; } @else { x: 4px; } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			'@if (list.index($_RESOURCES, "clean") != null) { @error "sdssd"; }',
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			'@if (index(m.$_RESOURCES, "clean") != null) { @error "sdssd"; }',
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			'@if (list.index(m.$_RESOURCES, "clean") != null) { @error "sdssd"; }',
			parser,
			parser._parseStylesheet.bind(parser),
		);
	});
	...
	test("@for", function () {
		const parser = new SCSSParser();
		assertNode(
			"@for $i from 1 to 5 { .item-#{$i} { width: 2em * $i; }  }",
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
		);
		assertNode(
			"@for $k from 1 + $x through 5 + $x {  }",
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
		);
		assertError(
		...
		assertError(
			...
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
			SCSSParseError.FromExpected,
		);
		assertError(
			"@for $i from 0 by 4 {}",
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
			SCSSParseError.ThroughOrToExpected,
		);
		assertError(
			"@for $i from {}",
			parser,
		...
		assertNode(
			'@for $i from 1 through 60 { $s: $i + "%"; }',
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
		);

		assertNode(
			"@for $k from 1 + m.$x through 5 + $x {  }",
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
		);
		assertNode(
			"@for $k from 1 + $x through 5 + m.$x {  }",
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
		);
		assertNode(
			"@for $k from 1 + m.$x through 5 + m.$x {  }",
			parser,
			parser._parseRuleSetDeclaration.bind(parser),
		);
	});
	...
	test("@mixin", function () {
		const parser = new SCSSParser();
		assertNode(
			"@mixin large-text { font: { family: Arial; size: 20px; } color: #ff0000; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@mixin sexy-border($color, $width: 1in) { color: black; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"@mixin foo ($i:4) { size: $i; @include wee ($i - 1); }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
	...
	test("@content", function () {
		...
		assertNode(
			"@content($type)",
			parser,
			parser._parseMixinContent.bind(parser),
		);
	});
	...
	test("@include", function () {
		...
		assertNode(
			"$values: #ff0000, #00ff00, #0000ff; .primary { @include colors($values...); }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			'@include colors(this("styles")...);',
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			".test { @include fontsize(16px, 21px !important); }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"p {  @include apply-to-ie6-only { #logo { background-image: url(/logo.gif); } } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"p { @include foo($values,) }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
		...
		assertError(
			"p { @include sexy-border($values blue",
			parser,
			parser._parseStylesheet.bind(parser),
			ParseError.RightParenthesisExpected,
		);
		...
		assertNode(
			"p { @include lib.sexy-border(blue); }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			".shadows { @include lib.box-shadow(0px 4px 5px #666, 2px 6px 10px #999); }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"$values: #ff0000, #00ff00, #0000ff; .primary { @include lib.colors($values...); }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			".primary { @include colors(lib.$values...); }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			".primary { @include lib.colors(lib.$values...); }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			'@include lib.colors(this("styles")...);',
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			'@include colors(lib.this("styles")...);',
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			'@include lib.colors(lib.this("styles")...);',
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			".test { @include lib.fontsize(16px, 21px !important); }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"p {  @include lib.apply-to-ie6-only { #logo { background-image: url(/logo.gif); } } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"p { @include lib.foo($values,) }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"p { @include foo(lib.$values,) }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"p { @include lib.foo(m.$values,); }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		...
		assertNode(
			'@include rtl("left") using ($dir) { margin-#{$dir}: 10px; }',
			parser,
			parser._parseStylesheet.bind(parser),
		);
	});
	...
	test("@function", function () {
		...
		assertNode(
			"@function grid-width($n: 1, $e) { @return 0; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@function foo($total, $a) { @for $i from 0 to $total { } @return $grid; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			'@function foo() { @if (unit($a) == "%") and ($i == ($total - 1)) { @return 0; } @return 1; }',
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@function is-even($int) { @if $int%2 == 0 { @return true; } @return false }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@function bar ($i) { @if $i > 0 { @return $i * bar($i - 1); } @return 1; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
	...
	test("@at-root", function () {
		...
		assertNode(
			"@at-root #main2 .some-class {  padding-left: calc( #{$a-variable} + 8px ); }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@media print { .page {  @at-root (without: media) { } } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(
			"@media print { .page {  @at-root (with: rule) { } } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
	});
	...
	test("Ruleset", function () {
		const parser = new SCSSParser();
		assertNode(
			".selector { prop: erty $const 1px; }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			".selector { prop: erty $const 1px m.$foo; }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
	...
	test("Nested Ruleset", function () {
		const parser = new SCSSParser();
		assertNode(
			".class1 { $const: 1; .class { $const: 2; three: $const; const: 3; } one: $const; }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			".class1 { $const: 1; .class { $const: m.$foo; } one: $const; }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			".class1 { > .class2 { & > .class4 { rule1: v1; } } }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			"foo { @at-root { display: none; } }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			'th, tr { @at-root #{selector-replace(&, "tr")} { border-bottom: 0; } }',
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			".foo { @supports(display: grid) { .bar { display: none; }}}",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			".foo { @supports(display: grid) { display: none; }}",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			".foo { @supports (position: sticky) { @media (min-width: map-get($grid-breakpoints, md)) { position: sticky; } }}",
			parser,
			parser._parseRuleset.bind(parser),
		); // issue #152
	});
	...
	test("Selector Interpolation", function () {
		...
		assertNode(
			"sans-#{serif} { a-#{1 + 2}-color-#{$attr}: blue; }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
		...
		assertNode(
			".sc-col#{module.$postfix}-2-1 { }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			"p.#{module.$name} { #{$attr}-color: blue; }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			"p.#{$name} { #{module.$attr}-color: blue; }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			"p.#{module.$name} { #{module.$attr}-color: blue; }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
			"sans-#{serif} { a-#{1 + 2}-color-#{module.$attr}: blue; }",
			parser,
			parser._parseRuleset.bind(parser),
		);
		assertNode(
	...
	test("Map", function () {
		const parser = new SCSSParser();
		assertNode(
			"(key1: 1px, key2: solid + px, key3: (2+3))",
			parser,
			parser._parseExpr.bind(parser),
		);
	...
	test("@font-face", function () {
		...
		assertNode(
			...
			parser,
			parser._parseFontFace.bind(parser),
		);
		assertNode(
		...
		assertNode(
			"@font-face { unicode-range: U+0021-007F, u+1f49C, U+4??, U+??????; }",
			parser,
			parser._parseFontFace.bind(parser),
		);
		assertError(
