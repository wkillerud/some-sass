import { CompletionItemKind, CompletionItem, MarkupKind, InsertTextFormat, CompletionItemTag, CompletionItemLabelDetails } from 'vscode-languageserver';
import { applySassDoc } from '../../utils/sassdoc';

import type { TextDocument } from 'vscode-languageserver-textdocument';
import type { IScssDocument, ScssFunction } from '../../types/symbols';
...
export function createFunctionCompletionItems(
	...
	for (let func of scssDocument.functions.values()) {
		...
		completions.push(makeFunctionCompletion(
			label,
			{
		...
		if (requiredParameters.length !== func.parameters.length) {
			const parametersSnippet = func.parameters.map(mapParameterSnippet).join(", ");
			const functionSignature = func.parameters.map(mapParameterSignature).join(", ");
			completions.push(makeFunctionCompletion(
				label,
				{
					detail: `(${functionSignature})`,
...
function makeFunctionCompletion(label: string, labelDetails: CompletionItemLabelDetails | undefined, filterText: string, sortText: string | undefined, detail: string, insertText: string, func: ScssFunction, documentation: string): CompletionItem {
	return {
		label,
		labelDetails,
		filterText,
