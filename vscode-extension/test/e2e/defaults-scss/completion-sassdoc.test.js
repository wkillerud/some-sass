...
test("Offers completions for SassDoc block on mixin without parameters or @content", async () => {
...
test("Offers completions for SassDoc block on mixin with @content", async () => {
	const expectedCompletions = [
		{
			label: "SassDoc Block",
			insertText: '" ${0}\\n/// @content ${1}\\n/// @output ${2}"',
...
test("Offers completions for SassDoc block on mixin with parameters", async () => {
	const expectedCompletions = [
		{
			label: "SassDoc Block",
			insertText:
...
test("Offers completions for SassDoc block on mixin with parameters and @content", async () => {
	const expectedCompletions = [
		{
			label: "SassDoc Block",
			insertText:
...
test("Offers completions for SassDoc block on parameterless function", async () => {
	const expectedCompletions = [
		{
			label: "SassDoc Block",
			insertText: '" ${0}\\n/// @return {${1:type}} ${2:-}"',
...
test("Offers completions for SassDoc block on parameterfull function", async () => {
	const expectedCompletions = [
		{
			label: "SassDoc Block",
			insertText:
