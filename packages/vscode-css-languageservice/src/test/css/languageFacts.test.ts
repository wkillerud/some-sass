...
export function assertColorValue(actual: Color | null, expected: Color | null, message: string) {
	if (actual && expected) {
...
suite("CSS - Language Facts", () => {
	...
	test("is color", function () {
		...
		assertColor(
			parser,
			"#main { color: rgb(100%, 34%, 10%, 50%) }",
			"rgb",
			colorFrom256RGB(255, 255 * 0.34, 255 * 0.1, 0.5),
		);
		assertColor(
			parser,
			"#main { color: rgba(+78, 40.6, 99%, 1% ) }",
			"rgb",
			colorFrom256RGB(78, 40.6, 255 * 0.99, 0.01),
		);
		assertColor(parser, "#main { color: hsl(120deg, 100%, 50%) }", "hsl", colorFrom256RGB(0, 255, 0));
		assertColor(
			parser,
			"#main { color: hsl(180,100%,25%, 0.33) }",
			"hsl",
			colorFrom256RGB(0, 0.5 * 255, 0.5 * 255, 0.33),
		);
	...
	test("xyzToRGB", function () {
		...
		assertColorValue(
			xyzToRGB({ x: 9.22, y: 15.58, z: 5.54, alpha: 1 }),
			{ red: 50, green: 125, blue: 50, alpha: 1 },
			"xyz(9.22, 15.58, 5.54)",
		);
		assertColorValue(
			xyzToRGB({ x: 9.8, y: 18.42, z: 3.53, alpha: 1 }),
			{ red: 35, green: 137, blue: 16, alpha: 1 },
			"2",
		);
	...
	test("XYZtoOKLAB", function () {
		// Verified with https://www.colorspaceconverter.com/converter/oklab-to-xyz
		assertLABValue(XYZtoOKLAB({ x: 8.8, y: 15.19, z: 5.24, alpha: 1 }), {
			l: 0.52,
			a: -0.110_09,
			b: 0.079_98,
			alpha: 1,
		});
		assertLABValue(XYZtoOKLAB({ x: 9.49, y: 18.2, z: 3.42, alpha: 1 }), {
			l: 0.549_98,
			a: -0.14,
			b: 0.109_94,
			alpha: 1,
		});
		assertLABValue(XYZtoOKLAB({ x: 70.1, y: 61.33, z: 71.52, alpha: 1 }), {
			l: 0.860_01,
			a: 0.079_99,
			b: -0.010_11,
			alpha: 1,
	...
	test("OKLABToRGB", function () {
		...
		assertColorValue(
			colorFromOKLAB(0.86, 0.08, -0.01),
			colorFrom256RGB(251.88, 187.65, 213.63),
			"oklab(86%, 0.08, -0.01)",
		);
	});

	test("labFromLCH", function () {
		assertLABValue(labFromLCH(46.41, 51.6, 139.5), { l: 46.41, a: -39.24, b: 33.51, alpha: 1 });
	});

	test("LCHtoRGB", function () {
		assertColorValue(colorFromLCH(46.41, 51.6, 139.5), colorFrom256RGB(50, 125, 50), "lch(46.41, 51.60, 139.50)");
	});

	test("OKLCHtoRGB", function () {
		// Verified with https://www.colorspaceconverter.com/converter/oklch-to-rgb
		assertColorValue(
			colorFromOKLCH(0.52, 0.13, 143.39),
			colorFrom256RGB(50.32, 123.2, 50.17),
			"oklch(52%, .13, 143.39)",
		);
	});
	...
	test("RGBToLCH", function () {
		assertLCHValue(lchFromColor(colorFrom256RGB(50, 125, 50)), { l: 46.41, c: 51.6, h: 139.5 });
	});
});
