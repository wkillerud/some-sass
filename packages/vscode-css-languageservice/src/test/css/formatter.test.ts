...
export function assertFormat(
	unformatted: string,
	expected: string,
	options: CSSFormatConfiguration = { tabSize: 2, insertSpaces: true },
	ls: LanguageService = getCSSLanguageService(),
) {
...
suite("CSS - Formatter", () => {
	test("full document", () => {
		const content = ["@font-face { src: url(http://test) }", '.monaco  .list { background: "#FFF"  ; }'].join("\n");
	...
	test("range", () => {
		...
		const expected = ["@font-face { src: url(http://test) }", ".monaco .list {", '  background: "#FFF";', "}"].join(
			"\n",
		);
	...
	test("range2", () => {
		const content = ["div {", "|  color:green|", "}"].join("\n");

		const expected = ["div {", "  color: green", "}"].join("\n");

		assertFormat(content, expected);
	});

	test("@media", () => {
		const content = ["@media print { @page { margin: 10% } blockquote, pre { page-break-inside: avoid } }"].join("\n");
	...
	test("selectors and functions", () => {
		const content = [".foo,.bar,li:first-of-type + li{--widthB:  calc(  const(--widthA)   / 2);}"].join("\n");

		const expected = [".foo,", ".bar,", "li:first-of-type+li {", "  --widthB: calc(const(--widthA) / 2);", "}"].join(
			"\n",
		);
	...
	test("insertFinalNewline", () => {
		const content = [".emptyMarkdownCell::before { outline:  1px  solid -webkit-focus-ring-color;  }"].join("\n");

		const expected = [".emptyMarkdownCell::before {", "  outline: 1px solid -webkit-focus-ring-color;", "}", ""].join(
			"\n",
		);
	...
	test("braceStyle", () => {
		const content = [".foo { display:  node;  }"].join("\n");

		const expected = [".foo", "{", "  display: node;", "}"].join("\n");

		assertFormat(content, expected, { insertSpaces: true, tabSize: 2, braceStyle: "expand" });

		const expected2 = [".foo {", "  display: node;", "}"].join("\n");
	...
	test("spaces", () => {
		...
		const content = [".body {", " font-size: @fs  !important; // 2 space -> BUG", "}"].join("\n");
