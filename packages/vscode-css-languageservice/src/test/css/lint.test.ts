...
export function assertEntries(
	...
	settings = new LintConfigurationSettings(),
): void {
	const entries = LintVisitor.entries(
		node,
		document,
		settings,
		cssDataManager,
		Level.Error | Level.Warning | Level.Ignore,
	);
	...
	for (const entry of entries) {
		...
		assert.ok(
			index !== -1,
			`${entry.getRule().id} found but not expected (${expectedRules.map((r) => r.id).join(", ")})`,
		);
		if (expectedMessages) {
...
suite("CSS - Lint", () => {
	...
	test("properies ignored due to inline ", function () {
		...
		assertRuleSet(
			"selector { display: inline-block; float: none !important; }",
			Rules.AvoidFloat,
			Rules.AvoidImportant,
		);
	});
	...
	test("unknown properties", function () {
		...
		assertRuleSet(
			'selector { -moz-box-shadow: "rest is missing" }',
		...
		assertRuleSet2(
			'selector { foo: "some"; bar: 0px }',
			[],
			undefined,
			new LintConfigurationSettings({ validProperties: ["foo", "bar"] }),
		);
		assertRuleSet2(
			'selector { foo: "some"; }',
			[],
			undefined,
			new LintConfigurationSettings({ validProperties: ["foo", null] }),
		);
		assertRuleSet2(
			'selector { bar: "some"; }',
			[Rules.UnknownProperty],
			undefined,
			new LintConfigurationSettings({ validProperties: ["foo"] }),
		);
		assertRuleSet2(
			'selector { Box-Property: "rest is missing" }',
			[Rules.UnknownProperty],
			["Unknown property: 'Box-Property'"],
		);
	});
	...
	test("box model", function () {
		...
		assertRuleSet(
			".mybox { height: 100px;         border: 1px;               }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		assertRuleSet(
			".mybox { height: 100px;         border: 1px solid;         }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		assertRuleSet(
			".mybox { width: 100px;          border: 1px;               }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		// order doesn't matter
		assertRuleSet(
			".mybox { border: 1px;           height: 100px;             }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		assertRuleSet(
			".mybox { border: 1px solid;     height: 100px;             }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		...
		assertRuleSet(
			".mybox { height: 100px;         border-top: 1px;           }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		); // shorthand | single value | 1px
		assertRuleSet(
			".mybox { height: 100px;         border-top: 1px solid;     }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		); // shorthand |
		...
		assertRuleSet(
			".mybox { height: 100px;         border-width: 1px;         }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		assertRuleSet(
			".mybox { height: 100px;         border-width: 0 0 1px;     }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		assertRuleSet(
			".mybox { width: 100px;          border-width: 0 1px;       }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		assertRuleSet(
			".mybox { width: 100px;          border-width: 0 0 0 1px;   }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		...
		assertRuleSet(
			".mybox { height: 100px;         border-style: solid;       }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		assertRuleSet(
			".mybox { height: 100px;         border-style: dashed;      }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		...
		assertRuleSet(
			".mybox { height: 100px;         border-top-width: 1px;     }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		...
		assertRuleSet(
			".mybox { height: 100px;         border-top-style: solid;   }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		...
		assertRuleSet(
			".mybox { height: 100px;         padding: 1px;              }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		assertRuleSet(
			".mybox { height: 100px;         padding: 1px 0;            }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		assertRuleSet(
			".mybox { height: 100px;         padding: 0 0 1px;          }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
		...
		assertRuleSet(
			".mybox { height: 100px;         padding: 1px;              border: }",
			Rules.BewareOfBoxModelSize,
			Rules.BewareOfBoxModelSize,
		);
	});
	...
	test("vendor specific prefixes", function () {
		...
		assertRuleSet(
			"selector { -moz-transform: none; transform: none; -o-transform: none; -webkit-transform: none; -ms-transform: none; }",
		);
		assertRuleSet("selector { --transform: none; }");
	...
	test("ignore missing standard properties in contexts with vendor specific pseudo-element", function () {
		...
		assertRuleSet(
			'input[type="range"]::-webkit-slider-thumb { color: black; & selector { -webkit-appearance: none; } }',
		);
		assertRuleSet(
