...
suite("LESS - Symbols", () => {
	...
	test("symbols in scopes", function () {
		...
		assertSymbolsInScope(
			...
			11,
			{ name: "@let", type: nodes.ReferenceType.Variable },
			{ name: ".class", type: nodes.ReferenceType.Rule },
		);
		assertSymbolsInScope(ls, "@let: iable; .class { @color: blue; }", 21, {
			name: "@color",
			type: nodes.ReferenceType.Variable,
		});
		assertSymbolsInScope(ls, "@let: iable; .class { @color: blue; }", 36, {
			name: "@color",
			type: nodes.ReferenceType.Variable,
		});
