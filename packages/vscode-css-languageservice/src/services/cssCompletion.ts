...
export class CSSCompletion {
	...
	public async doComplete2(
		...
		const participant: PathCompletionParticipant = new PathCompletionParticipant(
	...
	private doesSupportMarkdown() {
		if (!isDefined(this.supportsMarkdown)) {
			...
			const documentationFormat =
				this.lsOptions.clientCapabilities.textDocument?.completion?.completionItem?.documentationFormat;
