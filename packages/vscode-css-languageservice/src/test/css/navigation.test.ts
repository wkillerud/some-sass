...
export function assertHighlights(
	ls: LanguageService,
	input: string,
	marker: string,
...
export function assertSymbolInfos(
	ls: LanguageService,
	input: string,
...
export function assertColorPresentations(ls: LanguageService, color: Color, ...expected: string[]) {
	...
	assert.deepEqual(
		result.map((r) => r.textEdit),
		expected.map((l) => TextEdit.replace(range, l)),
	);
}
...
export function assertScopeBuilding(
	ls: LanguageService,
	input: string,
	...scopes: { offset: number; length: number }[]
): void {
	const global = createScope(ls, input);
...
suite("CSS - Navigation", () => {
	suite("Scope", () => {
		...
		test("symbols in scopes", function () {
			const ls = getCSSLS();
			assertSymbolsInScope(ls, "@keyframes animation {};", 0, {
				name: "animation",
				type: nodes.ReferenceType.Keyframe,
			});
			assertSymbolsInScope(
				ls,
				" .class1 {} .class2 {}",
				0,
				{ name: ".class1", type: nodes.ReferenceType.Rule },
				{ name: ".class2", type: nodes.ReferenceType.Rule },
			);
		});
		...
		test("test variables in root scope", function () {
			...
			assertSymbolsInScope(
				ls,
				":root{ --var1: abc; --var2: def; }",
				0,
				{ name: "--var1", type: nodes.ReferenceType.Variable },
				{ name: "--var2", type: nodes.ReferenceType.Variable },
			);
		});
		...
		test("test variables in local scope get root variables and other local variables too", function () {
			...
			assertSymbolsInScope(
				ls,
				".a{ --var1: abc; } .b{ --var2: abc; } :root{ --var3: abc;}",
				2,
				{ name: "--var1", type: nodes.ReferenceType.Variable },
				{ name: "--var2", type: nodes.ReferenceType.Variable },
				{ name: "--var3", type: nodes.ReferenceType.Variable },
			);
		});
	});
	...
	suite("Symbols", () => {
		test("basic symbol infos", () => {
			const ls = getCSSLS();
			assertSymbolInfos(ls, ".foo {}", [
				{ name: ".foo", kind: SymbolKind.Class, location: Location.create("test://test/test.css", newRange(0, 7)) },
			]);
			assertSymbolInfos(ls, ".foo:not(.selected) {}", [
				{
					name: ".foo:not(.selected)",
					kind: SymbolKind.Class,
					location: Location.create("test://test/test.css", newRange(0, 22)),
				},
			]);
			...
			assertSymbolInfos(ls, ".voo.doo, .bar {}", [
				{
					name: ".voo.doo",
					kind: SymbolKind.Class,
					location: Location.create("test://test/test.css", newRange(0, 17)),
				},
				{ name: ".bar", kind: SymbolKind.Class, location: Location.create("test://test/test.css", newRange(10, 17)) },
			]);

			// Media Query
			assertSymbolInfos(ls, "@media screen, print {}", [
				{
		...
		test("basic document symbols", () => {
			...
			assertDocumentSymbols(ls, ".foo {}", [
				{ name: ".foo", kind: SymbolKind.Class, range: newRange(0, 7), selectionRange: newRange(0, 4) },
			]);
	...
	suite("Links", () => {
		...
		test("No links for data:", async () => {
			...
			await assertLinks(
				ls,
				`body { background-image: url(data:image/gif;base64,R0lGODlhEAAQAMQAAORHHOVSKudfOul) }`,
				[],
			);
		});
		...
		test("url links", async function () {
			...
			await assertLinks(
				...
				[{ range: newRange(29, 41), target: getTestResource("hello.html") }],
				"css",
				testUri,
				workspaceFolder,
			);
	...
	suite("Color", () => {
		test("color symbols", function () {
			...
			assertColorSymbols(ls, "body { backgroundColor: #ff9977; }", {
				color: colorFrom256RGB(0xff, 0x99, 0x77),
				range: newRange(24, 31),
			});
			assertColorSymbols(ls, "body { backgroundColor: hsl(0, 0%, 100%); }", {
				color: colorFrom256RGB(255, 255, 255),
				range: newRange(24, 40),
			});
			...
			assertColorSymbols(ls, "body { backgroundColor: rgba(1, 40, 1, 0.3); }", {
				color: colorFrom256RGB(1, 40, 1, 0.3),
				range: newRange(24, 43),
			});
			assertColorSymbols(ls, "body { backgroundColor: hwb(194 0% 0% / .5); }", {
				color: colorFromHWB(194, 0, 0, 0.5),
				range: newRange(24, 43),
			});
		});
