...
export class Parser {
	...
	public _completeParseImport(node: nodes.Import): nodes.Node | null {
		...
		if (this.acceptIdent("supports")) {
			if (this.accept(TokenType.ParenthesisL)) {
				node.addChild(
					this._tryToParseDeclaration() || this._parseSupportsCondition(),
				);
				if (!this.accept(TokenType.ParenthesisR)) {
					return this.finish(
						node,
	...
	private _parseSupportsConditionInParens(): nodes.Node {
		...
		if (this.accept(TokenType.ParenthesisL)) {
			...
			if (
				!node.addChild(this._tryToParseDeclaration([TokenType.ParenthesisR]))
			) {
	...
	public _parseSimpleSelector(): nodes.SimpleSelector | null {
		...
		let c = 0;
		if (
			node.addChild(this._parseElementName() || this._parseNestingSelector())
		) {
			c++;
		}
		while (
