import { CompletionItem, CompletionItemKind, CompletionItemLabelDetails, CompletionItemTag, InsertTextFormat, MarkupKind } from 'vscode-languageserver';
...
import { makeMixinDocumentation, mapParameterSignature, mapParameterSnippet, rePrivate } from './completion-utils';
...
export function createMixinCompletionItems(
	...
	for (let mixin of scssDocument.mixins.values()) {
		...
		if (requiredParameters.length === 0) {
			makeMixinCompletion(completions, label, labelDetails, filterText, sortText, detail, insertText, mixin, documentation);
		}

		if (requiredParameters.length) {
			const parametersSnippet = requiredParameters.map(mapParameterSnippet).join(", ");
			const functionSignature = requiredParameters.map(mapParameterSignature).join(", ");
			makeMixinCompletion(
				completions,
				label,
				{
		...
		if (mixin.parameters.length !== requiredParameters.length) {
			const parametersSnippet = mixin.parameters.map(mapParameterSnippet).join(", ");
			const functionSignature = mixin.parameters.map(mapParameterSignature).join(", ");
			makeMixinCompletion(
				completions,
				label,
				{
...
function makeMixinCompletion(completions: CompletionItem[], label: string, labelDetails: CompletionItemLabelDetails | undefined, filterText: string, sortText: string | undefined, detail: string, insertText: string, mixin: ScssMixin, documentation: string): void {
	completions.push({
		label,
		labelDetails,
		filterText,
