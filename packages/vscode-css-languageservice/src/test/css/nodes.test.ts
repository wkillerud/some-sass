...
suite("CSS - Nodes", () => {
	...
	test("RuleSet", function () {
		assertNodes(
			ruleset,
			"selector{prop:value}",
			"ruleset,...,selector,simpleselector,elementnameselector,identifier,declarations,declaration,property,...",
		);
		assertNodes(
			ruleset,
			"selector { prop: value }",
	...
	test("Keyframe", function () {
		...
		assertNodes(
			fn,
			"@keyframes name { from { top: 0px} to { top: 100px } }",
			"keyframe,identifier,...,keyframeselector,...,declaration,...,keyframeselector,...,declaration,...",
		);
	});
	...
	test("UnicodeRange", function () {
		...
		assertNodes(
			fn,
			"@font-face { unicode-range: U+0020-01ff, U+1?? }",
			"fontface,declarations,declaration,property,identifier,expression,binaryexpression,term,unicoderange,...",
		);
	});
	...
	test("Stylesheet", function () {
		...
		assertNodes(
			stylesheet,
			"selector { .foo {} }",
			"stylesheet,ruleset,...,selector,...,elementnameselector,...,ruleset,...,selector,...,classselector,...",
		);
		assertNodes(
			stylesheet,
			"selector { :hover {} }",
			"stylesheet,ruleset,...,selector,...,elementnameselector,...,ruleset,...,selector,...,pseudoselector,...",
		);
		assertNodes(
			stylesheet,
			"selector { :hover {}; }",
		...
		assertNodes(
			stylesheet,
			"selector { [value] {} }",
			"stylesheet,ruleset,...,selector,...,elementnameselector,...,ruleset,...,selector,...,attributeselector,...",
		);
		assertNodes(
			stylesheet,
			"selector { & div {} }",
			"stylesheet,ruleset,...,selector,...,elementnameselector,...,ruleset,...,selector,...,selectorcombinator,...,elementnameselector,...",
		);
		assertNodes(
			stylesheet,
			"selector { .foo { color: blue; } }",
			"stylesheet,ruleset,...,selector,...,elementnameselector,...,ruleset,...,selector,...,classselector,...,declaration,property,identifier,...",
		);
		assertNodes(
			stylesheet,
			"selector { @media screen { color: blue; } }",
			"stylesheet,ruleset,...,selector,...,elementnameselector,...,media,...,mediaquery,...,declaration,property,identifier,...",
		);
		assertNodes(
			stylesheet,
			"selector { @supports (width: 20rx) { color: blue; } }",
			"stylesheet,ruleset,...,selector,...,elementnameselector,...,supports,...,declarations,declaration,property,...",
		);
		assertNodes(
			stylesheet,
			"selector { @layer foo { color: blue; } }",
			"stylesheet,ruleset,...,selector,...,elementnameselector,...,layer,...,declarations,declaration,property,...",
		);
	});
});
