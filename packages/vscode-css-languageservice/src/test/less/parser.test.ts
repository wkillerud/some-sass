...
suite("LESS - Parser", () => {
	...
	test("MixinDeclaration", function () {
		...
		assertNode(
			...
			parser,
			parser._tryParseMixinDeclaration.bind(parser),
		);
	...
	test("MixinReference", function () {
		...
		assertError(
			".mixin(#008000;;)",
			parser,
			parser._tryParseMixinReference.bind(parser),
	...
	test("DetachedRuleSet", function () {
		...
		assertNode(
			".media-switch(@styles) { @media(orientation:landscape){ @styles(); @foo: 9; } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode(".media-switch({ flex-direction: row; });", parser, parser._parseStylesheet.bind(parser));
		assertNode(
			".keyframes(@name; @arguments) { @-moz-keyframes @name { @arguments(); } }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
	...
	test("Nested Ruleset", function () {
		...
		assertNode(
			"@supports (property: value) { .outOfMedia & { @media (max-size: 2px) { @supports (whatever: something) { property: value;}}}}",
			parser,
			parser._parseStylesheet.bind(parser),
		);
	});
	...
	test("CSS Guards", function () {
		const parser = new LESSParser();
		assertNode(
			".selector when not ( @testCondition = 2) and not ( @testCondition = 3 ) { }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
		assertNode("button when (@my-option = true) { color: white; }", parser, parser._parseStylesheet.bind(parser));
		assertNode(
			".something .other when (@my-option = true) { color: white; }",
			parser,
			parser._parseStylesheet.bind(parser),
		);
