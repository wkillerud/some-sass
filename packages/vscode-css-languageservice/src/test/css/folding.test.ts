...
suite("CSS Folding - Basic", () => {
	test("Fold single rule", () => {
		const input = [/*0*/ ".foo {", /*1*/ "  color: red;", /*2*/ "}"];
	...
	test("Fold multiple rules", () => {
		const input = [/*0*/ ".foo {", /*1*/ "  color: red;", /*2*/ "  opacity: 1;", /*3*/ "}"];
...
suite("CSS Folding - Partial", () => {
	test("Without closing curly brace", () => {
		const input = [/*0*/ ".foo {", /*1*/ "color: red;"];
		assertRanges(input, []);
	});

	test("Without closing curly brace creates correct folding ranges", () => {
		const input = [/*0*/ ".foo {", /*1*/ "color: red;", /*2*/ ".bar {", /*3*/ "color: blue;", /*4*/ "}"];
	...
	test("Without opening curly brace should not throw error", () => {
		const input = [/*0*/ ".foo", /*1*/ "  color: blue;", /*2*/ "}}"];
		assertRanges(input, []);
	});
...
suite("CSS Folding - Comments", () => {
	test("Comment - single star", () => {
		const input = [/*0*/ "/*", /*1*/ ".foo {", /*2*/ "  color: red;", /*3*/ "}", /*4*/ "*/"];
	...
	test("Comment - wrong indentation and no newline", () => {
		const input = [/*0*/ "/**", /*1*/ ".foo{", /*2*/ "color: red;", /*3*/ "} */"];
...
suite("CSS Folding - Nested", () => {
	test("Postcss nested", () => {
		const input = [/*0*/ ".foo {", /*1*/ "& .bar {", /*2*/ "  color: red;", /*3*/ "}", /*4*/ "}"];
		assertRanges(input, [r(0, 3), r(1, 2)]);
	});

	test("Media query", () => {
		const input = [/*0*/ "@media screen {", /*1*/ ".foo {", /*2*/ "color: red;", /*3*/ "}", /*4*/ "}"];
		assertRanges(input, [r(0, 3), r(1, 2)]);
	});
});
...
suite("CSS Folding - No intersections and always choose first region", () => {
	...
	test("declaration intersecting with region", () => {
		const input = [/*0*/ ".bar {", /*1*/ "/* #region */", /*2*/ "  color: red;", /*3*/ "}", /*4*/ "/* #endregion */"];
		assertRanges(input, [r(0, 2)]);
	});
});
...
suite("SCSS Folding", () => {
	...
	test("SCSS Interolation", () => {
		const input = [/*0*/ ".orbit-#{$d}-prev {", /*1*/ "  foo-#{$d}-bar: 1;", /*2*/ "  #{$d}-bar-#{$d}: 2;", /*3*/ "}"];
